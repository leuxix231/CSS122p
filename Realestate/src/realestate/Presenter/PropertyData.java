...
public class PropertyData {
    ...
    public void displayLots(List<Property> properties) {
        System.out.printf("%-10s %-8s %-6s %-8s %-10s %-10s%n",
    ...
    public List<Property> filterBySize(List<Property> properties, int size) {
        ...
        for (Property p : properties) {
            if (p.getArea() == size) result.add(p);
        }
        return result;
    }

    // ─── Filter by block (String: A, B, C, D, E) ───────────────────────────────
    public List<Property> filterByBlock(List<Property> properties, String block) {
        List<Property> result = new ArrayList<>();
        for (Property p : properties) {
            if (p.getBlock().equalsIgnoreCase(block)) result.add(p);
        }
        return result;
    }
